//
// ASCOM.DomeDriverServer Local COM Server
//
...
namespace ASCOM.DomeDriverServer
{
    public static class Server
    {
        ...
        private static string s_appId = "{57e7431b-24dc-44ac-a982-f44e58dd1ed2}";	// Our AppId
            ...
            foreach (FileInfo fi in d.GetFiles("*.dll"))
            {
                ...
                catch (Exception e)
                {
                    MessageBox.Show("Failed to load served COM class assembly " + fi.Name + " - " + e.Message,
                        "DomeDriverServer", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                    return false;
                }
        ...
        private static void ElevateSelf(string arg)
        {
            ...
            {
                MessageBox.Show("The DomeDriverServer was not " + (arg == "/register" ? "registered" : "unregistered") +
                    " because you did not allow it.", "DomeDriverServer", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.ToString(), "DomeDriverServer", MessageBoxButtons.OK, MessageBoxIcon.Stop);
            }
            return;
        }
                ...
                switch (args[0].ToLower())
                {
                    ...
                    default:
                        MessageBox.Show("Unknown argument: " + args[0] + "\nValid are : -register, -unregister and -embedding",
                            "DomeDriverServer", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                        break;
                }
            }
            else
