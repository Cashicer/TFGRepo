//
// ASCOM.TalonSuccessor Local COM Server
//
...
namespace ASCOM.TalonSuccessor
{
    public static class Server
    {
        ...
        private static string s_appId = "{0caa752d-2c5c-40bb-ad7f-1da0f65f4adb}";	// Our AppId
            ...
            foreach (FileInfo fi in d.GetFiles("*.dll"))
            {
                ...
                catch (Exception e)
                {
                    MessageBox.Show("Failed to load served COM class assembly " + fi.Name + " - " + e.Message,
                        "TalonSuccessor", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                    return false;
                }
        ...
        private static void ElevateSelf(string arg)
        {
            ...
            {
                MessageBox.Show("The TalonSuccessor was not " + (arg == "/register" ? "registered" : "unregistered") +
                    " because you did not allow it.", "TalonSuccessor", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.ToString(), "TalonSuccessor", MessageBoxButtons.OK, MessageBoxIcon.Stop);
            }
            return;
        }
                ...
                switch (args[0].ToLower())
                {
                    ...
                    default:
                        MessageBox.Show("Unknown argument: " + args[0] + "\nValid are : -register, -unregister and -embedding",
                            "TalonSuccessor", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                        break;
                }
            }
            else
